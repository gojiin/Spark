  ...
  def main(args: Array[String]): Unit = {
        ...
        var groupRddMapExp = mapRdd.
          ...
          flatMap(x=>{
            ...
            // 1. Sort Data
            //Row형태 데이터로 Seq로 나래비를 세워서 뽑아줌
            //sortBy하면 내가 어떤 형태로 정렬을 할거라는 뜻
            var sortedData = data.toSeq.sortBy(x=>{x.getString(yearweekNo).toInt}) //yearWeekNo의 getString값으로 정렬할거야(올림차순)
            // 내림차순으로 정렬 : 앞에 -붙여서
            //            var sortedData = data.toSeq.sortBy(x=>{-(x.getString(yearweekNo)).toInt})
            // 인덱스를 뒤에서 생성하여 정렬 시켜주는 키워드 : zipWithIndex
            ...
            //Buffer는 Row형태도 있고, 타입을 다 정한 Buffer도 있음
            ...
            // 4. Merge all data-set for moving average
            //기존데이터에 ++g하면 그게 union임! (단, 컬럼갯수가 같아야 함)
        ...
        //    finalResultDf.
        //      coalesce(1). // ÆAAI°³¼o
        //      write.format("csv").  // AuAaÆ÷¸E
        //      mode("overwrite"). // AuAa¸ðμa append/overwrite
        //      save("season_result") // AuAaÆAAI¸i
